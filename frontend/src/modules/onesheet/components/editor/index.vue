<!--
  ...
  `usePivotIntegration`, `useExportImport` and the rest of this directory — and
  what is left is the part that does not decompose: one canvas, one selection,
  one history, and forty functions that each read a dozen of the others. Pulling
  those apart makes modules that take fifteen injected dependencies, which is
  the same coupling written twice. The stylesheet is in `editor.css` and
  `editor.global.css`, because CSS has no closure to share.

  The template, in order: the load-time error state, then Bar 1 (identity), Bar
  2 (formatting), Bar 3 (the formula bar), the version banner, the canvas and
  its overlays, the add-rows strip, the bottom bar of sheet tabs, and then every
  dialog and floating panel.
-->
<template>
  <div class="sn-root">
...
import { createClipboard }     from '@/modules/onesheet/lib/engine/clipboard.js'
...
import { createCommentsEngine }  from '@/modules/onesheet/lib/engine/comments.js'
...
import { chipColor } from '@/modules/onesheet/lib/canvas/chip-geometry.js'
import { createCondFormatEngine } from '@/modules/onesheet/lib/engine/cond-format.js'
import { detectHyperlink, isAutoLinkText } from '@/modules/onesheet/lib/engine/links.js'
import { fetchLinkPreview } from '@/modules/onesheet/lib/services/linkPreview.js'
...
import { useContextMenu } from '@/modules/onesheet/components/editor/useContextMenu.js'
...
import FindReplace             from '@/modules/onesheet/components/editor/FindReplace.vue'
import VersionHistory          from '@/modules/onesheet/components/editor/VersionHistory.vue'
import VersionPreviewBanner    from '@/modules/onesheet/components/editor/VersionPreviewBanner.vue'
import CellHistoryPopover      from '@/modules/onesheet/components/editor/CellHistoryPopover.vue'
import SplitTextPopover        from '@/modules/onesheet/components/editor/SplitTextPopover.vue'
import LinkPreviewCard         from '@/modules/onesheet/components/editor/LinkPreviewCard.vue'
import PivotDialog             from '@/modules/onesheet/components/editor/PivotDialog.vue'
import ColorPicker             from '@/modules/onesheet/components/editor/ColorPicker.vue'
...
import { useChartIntegration } from '@/modules/onesheet/components/editor/useChartIntegration.js'
import ChartDialog             from '@/modules/onesheet/components/editor/ChartDialog.vue'
import ChartOverlay            from '@/modules/onesheet/components/editor/ChartOverlay.vue'
...
<style src="./editor.global.css"></style>
