 ...
 *
 * The letters are Gmail's — j, k, e, u, s, c — because every client converged
 * on them, and a product that picks different ones asks people to learn
 ...
 *   text is left alone, as is any key pressed with Ctrl or Meta held unless the
 *   shortcut asked for it.
 * * **Never over a dialog.** `document.querySelector('[role=dialog]')` is how
 *   that is known, because it is what every frappe-ui overlay renders.
 *
...
export function pressed(event) {
  ...
  // different shortcut from `u`, but `?` is already shifted.
 ...
 * shortcut that also scrolls the page half worked. One that returns `false`
 * says it did not handle this after all.
 */
