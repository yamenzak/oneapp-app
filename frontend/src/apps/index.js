 ...
 * shape comes out of the manifest with no code here — a view names a doctype and
 ...
 * anything a list cannot be. A view whose `component` matches a key below gets
 * that component instead, with the app code and view slug as props, and nothing
 * else about the view applies.
 *
 * Keyed `appCode/view` so two apps can each have an `overview` and neither has
...
export function appComponent(name) {
