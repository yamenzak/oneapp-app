 ...
 * chances to disagree. See `onecalendar/diary.py`.
 */
...
export const diary = {
  ...
  // `diary._mine`.
  diaryEvent: (name) =>
    callMethod('oneapp.onecalendar.diary.event', { name }, { silent: true, method: 'GET' }),

  saveDiaryEvent: (values) =>
    callMethod('oneapp.onecalendar.diary.save_event', { values: JSON.stringify(values) }),

  removeDiaryEvent: (name) =>
    callMethod('oneapp.onecalendar.diary.remove_event', { name }),
}
