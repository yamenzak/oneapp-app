 ...
 * rather than two copies of this file. See `shared/versions.py`.
 *
...
export const versions = {
  fileHistory: (file, kind) =>
    callMethod('oneapp.shared.versions.history',
      { file, kind, offset_minutes: OFFSET() },
      { silent: true, method: 'GET' }),

  fileVersionBody: (version, kind) =>
    callMethod('oneapp.shared.versions.version_body', { version, kind }, {
      silent: true, method: 'GET',
    }),

  fileKeepVersion: (file, kind, title) =>
    callMethod('oneapp.shared.versions.save_version', { file, kind, title }, {
      success: 'Version saved',
    }),

  fileRestoreVersion: (version, kind) =>
    callMethod('oneapp.shared.versions.restore_version', { version, kind }, {
      success: 'Restored',
    }),

  fileNameVersion: (version, kind, title) =>
    callMethod('oneapp.shared.versions.name_version', { version, kind, title }, {
      success: 'Renamed',
    }),

  fileForgetVersion: (version, kind) =>
    callMethod('oneapp.shared.versions.forget_version', { version, kind }, {
