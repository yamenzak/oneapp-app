...
from oneapp.onespace import branding, theming

# Set by the control plane's sync. An Admin member holds it too — see
# onespace/sync.py, which is deliberately the only thing that grants it.
...
class Setting:
	...
	def __init__(self, key, label, type="Data", targets=(), default_key="",
		...
		# accent is the first — `onespace/branding.py` says why there is no
...
GROUPS = [
	{
		...
		"settings": [
			...
			Setting(
				...
				# `onespace/branding.py` for why Website Theme's own
...
def save(group: str, values: str | dict) -> dict:
	...
	from oneapp.onestorage.writing import publish
	...
	# to a second field — see `onespace/branding.py`. Unconditional within
...
# `onespace.naming`, which is a gate and a shape over Frappe's own
...
# to the sentence somebody would say out loud. See `onespace/alerts.py`.
# --------------------------------------------------------------------------- #
...
#
# Every mechanic lives in `onespace.printing`, over Frappe's own Print
