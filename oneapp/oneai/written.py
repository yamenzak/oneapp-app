...
def _marked_doctypes() -> set[str]:
	...
	if cached is None:
		# Not cached when the table is missing, which is a real state rather
		# than an error: this is hooked on `*`, so it runs during the migrate
		# that is still creating it. An uncached answer costs one query on the
		# next save; a cached empty set would outlive the migrate and make
		# every mark on the site invisible until something cleared it.
...
def written(doctype: str, name: str) -> dict[str, dict]:
	...
	from oneapp.oneai.settings import catalogue
