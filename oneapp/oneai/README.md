...
file is about the module itself: what it is made of, and the decisions in it
that cost something.

**Two halves, and the line between them is load-bearing.** The files beside
this one are the **spine**, and the spine may not import a module: a gateway
that knew about mail is a gateway mail could break, and the sixth surface would
find half of it already shaped around a mailbox. `chat/` is the **assistant**,
which is one `@ai_feature` that loops, and it *must* reach into modules —
answering "what is in my drive" means reading the drive.

That line used to come free from the directories, when the spine was
`onespace/ai/` and the assistant `onespace/chat/`. Both are OneAI's now, so it
is drawn by hand: `tests/test_ai_layering.py` holds every file beside this one
to the rule and names `chat/` as the surface that is exempt.
