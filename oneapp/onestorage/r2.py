...
def upload(file_doc, content: bytes) -> str:
	...
	if file_doc.is_private:
		...
		return f"/api/method/oneapp.onestorage.r2.download?file={file_doc.name}"
...
def public_url(file_doc, key: str) -> str:
	...
	return f"{base}/{key}" if base else (
		f"/api/method/oneapp.onestorage.r2.download?file={file_doc.name}"
	)
...
def serve(doc):
	...
	if doc.get("custom_kind") == "Sheet":
		from oneapp.onesheet import export

		export.to_response(doc)
		return

	if doc.get("custom_kind") == "Doc":
		from oneapp.onedoc import export as doc_export
